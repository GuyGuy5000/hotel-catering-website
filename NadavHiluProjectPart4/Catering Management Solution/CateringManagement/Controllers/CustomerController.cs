...
namespace CateringManagement.Controllers
{
    ...
    public class CustomerController : ElephantController
    {
        ...
        public CustomerController(CateringContext context, IMyEmailSender myEmailSender)
        {
            _context = context;
        ...
        public async Task<IActionResult> MarketingEmail(string[] selectedOptions, string Subject, string emailContent)
        {
            PopulateCustomerLists(selectedOptions);

            if (selectedOptions.Length == 0)
            {
                ViewData["Message"] = "Select at least one customer to send an email to.";
            }


            if (string.IsNullOrEmpty(Subject) || string.IsNullOrEmpty(emailContent))
            {
                ViewData["Message"] = "Please enter both a Subject and some message Content before sending the message.";
            }
            else
            {
                int customerCount = 0;
                try
                {
                    ...
                    List<EmailAddress> customers = _context.Customers
                                                           .Where(c => selectedOptions.Contains(c.ID.ToString()))
                                                           .Select(c => new EmailAddress { Name = c.FullName, Address = c.Email })
                                                           .ToList();
                    customerCount = customers.Count;

                    //gets test addresses then removes any addresses that aren't used for testing (instructor's emails commented out for testing)
                    List<EmailAddress> testCustomers = customers.Where(c => //c.Address == "jkaluba@niagaracollege.ca"|| 
                                                                            //c.Address == "dstovel@niagaracollege.ca"|| 
                                                                            c.Address == "nhilu1@ncstudents.niagaracollege.ca")
                                                                .Select(c => c)
                                                                .ToList();
                    customers.Clear();
                    customers.AddRange(testCustomers);
                    ...
                    if (customerCount > 0)
                    {
                        var msg = new EmailMessage()
                        {
                            ...
                            Subject = Subject,
                            Content = "<p>" + emailContent + "</p><p>Message automatically sent by <strong>Toronto Hotel - Catering Management</strong>.</p>"

                        };
                        await _myEmailSender.SendToManyAsync(msg);
                        ViewData["Message"] = "Message sent to " + customerCount + " Customer"
                            + ((customerCount == 1) ? "." : "s.");
                    }
                    else
                    {
                        ViewData["Message"] = "Message NOT sent!  No customers in the select list.";
                    }
                }
                catch (Exception)
                {
                    ViewData["Message"] = "Error: Could not send email message to " + customerCount + " Customer"
                        + ((customerCount == 1) ? "" : "s") + ".";
                }
            }
            return View();
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            if (User.IsInRole("Staff"))
            {
                ...
                {
                    ModelState.AddModelError("", "As a Supervisor, you cannot edit this " +
        ...
        private void PopulateCustomerLists(string[] selectedOptions)
        {
            var allOptions = _context.Customers;
            var currentOptionsHS = selectedOptions;
            ...
            foreach (var c in allOptions)
            {
                ...
                if (currentOptionsHS.Contains(c.FullName))
                {
